<?php 
// include("administrator/Cenfarmdbconnect.php");
...
if(isset($_REQUEST['action']) && !empty($_REQUEST['action'])){ 
    if($_REQUEST['action'] == 'addToCart' && !empty($_REQUEST['id'])){ 
        ...
        // Get product details 
        $query = $db->query("SELECT * FROM menu WHERE id = ".$menuID); 
        ...
        $redirectLoc = $insertItem?'menu.php':'viewCart.php'; 
        ...
        $addres = strip_tags($_POST['address']); 
        ...
        if(empty($errorMsg)){ 
            ...
            $insertCust = $db->query("INSERT INTO customers (first_name, last_name, email, phone_number, address) VALUES ('".$first_name."', '".$last_name."', '".$email."', '".$phone."', '".$addres."')"); 
            ...
            if($insertCust){ 
                ...
                if($insertOrder){ 
                    ...
                    foreach($cartItems as $item){ 
                        $sql .= "INSERT INTO order_items (order_id, menu_id, quantity) VALUES ('".$orderID."', '".$item['id']."', '".$item['qty']."');"; 
                    } 
